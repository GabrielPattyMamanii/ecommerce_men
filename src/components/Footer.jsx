...
export default function Footer() {
  ...
  return (
    <footer className="w-full border-t border-white/10 bg-background-dark pb-8 relative mt-auto overflow-hidden pt-12">
        ...
        <div className="flex flex-col lg:flex-row justify-between gap-12">
          ...
          <div className="grid grid-cols-2 gap-x-16 gap-y-8">
            ...
            <div className="flex flex-col gap-4">
              ...
              {categories.length === 0 ? (
                <Link className="text-[11px] text-slate-400 hover:text-primary transition-colors font-mono uppercase tracking-wider tech-underline w-fit" to="/catalogo">Ver catálogo</Link>
              ) : (
                categories.map(category => (
                  <Link
                    key={category.id}
                    className="text-[11px] text-slate-400 hover:text-primary transition-colors font-mono uppercase tracking-wider tech-underline w-fit"
        ...
        <div className="pt-6 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-5">
          ...
          <div className="flex gap-5 items-center">
            ...
            {contact?.whatsapp_url && (
              <a className="text-slate-400 hover:text-primary transition-all transform hover:scale-110" href={contact.whatsapp_url} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp">
            ...
            {contact?.tiktok_url && (
              <a className="text-slate-400 hover:text-primary transition-all transform hover:scale-110" href={contact.tiktok_url} target="_blank" rel="noopener noreferrer" aria-label="TikTok">
            ...
            {contact?.instagram_url && (
              <a className="text-slate-400 hover:text-primary transition-all transform hover:scale-110" href={contact.instagram_url} target="_blank" rel="noopener noreferrer" aria-label="Instagram">
            ...
            {contact?.facebook_url && (
              <a className="text-slate-400 hover:text-primary transition-all transform hover:scale-110" href={contact.facebook_url} target="_blank" rel="noopener noreferrer" aria-label="Facebook">
